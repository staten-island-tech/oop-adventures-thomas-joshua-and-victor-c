...
def play_game():
    ...
    while J.HP > 0 and enemy.HP > 0:
        print(f"\n--- Turn {turns + 1} ---")
        control = input("Move? (punch[p]/guard[g]): ").lower()
        ...
        if EnemySelect == 1:
            enemy_consecutive_guards = 0
            if PlayerGuard:
                damage = random.randint(1, enemy.attack)  # Calculate blocked damage
                print(f"{J.name}'s guard blocked {damage} damage from {enemy.name}!")
            else:
                enemy.EnemyPunch(J)
        else:
            enemy.EGuard()
